...
def migrate(main_root, rb_root, photo_path):
    ...
    for old_room_id, old_room in rb_root['Rooms'].iteritems():
        ...
        #     k = RoomAttributeKey.getAttributeByName(old_custom_attribute['name'])
        #     if not k:
        #         k = RoomAttributeKey(name=old_custom_attribute['name'])
        #     a = RoomAttribute(key=k)
        #     a.value = {''}
        ...
        if hasattr(old_room, 'customAtts'):
            for k, v in old_room.customAtts.iteritems():
                pprint(k)
                pprint(v)
                # s.add(k)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-p', '--photo', help='photos path', default=None)
