...
class ProtectionTest(LoggedInSeleniumTestCase):
    def setUp(self):
        LoggedInSeleniumTestCase.setUp(self)

    def test_general_settings_test(self):
        sel = self._selenium
        sel.open("/confModifAC.py?confId=62")
        ...
        sel.click("//input[@value='Add user to list' and @value='Add user to list' and @type='submit' and @onclick=\"this.form.action='http://localhost/confModifAC.py/selectRegistrars';\"]")
        sel.wait_for_page_to_load("30000")
        sel.type("surname", "dummy")
        sel.click("action")
        sel.wait_for_page_to_load("30000")
