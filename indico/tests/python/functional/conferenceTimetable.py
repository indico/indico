...
class TimetableTest(LoggedInSeleniumTestCase):
    def setUp(self):
        LoggedInSeleniumTestCase.setUp(self)

    def test_general_settings_test(self):
        sel = self._selenium
        sel.open("/confModifSchedule.py?confId=62#20110714")
        sel.click("link=Add new")
        sel.click("link=Session")
        for i in range(60):
            try:
                if sel.is_element_present("//div/input"): break
            except: pass
            time.sleep(1)
        else: self.fail("time out")
        sel.type("//div/input", "fvdsf")
        sel.click("css=div.popupButtonBar > div > input[type=button]")
        sel.click("link=Add new")
        sel.click("link=Contribution")
        for i in range(60):
            try:
                if sel.is_element_present("link=Create a new one"): break
            except: pass
            time.sleep(1)
        else: self.fail("time out")
        sel.click("//div/div[3]/div/div/div/ul/li/a")
        for i in range(60):
            try:
        ...
        sel.type("addContributionFocusField", "fewfre")
        ...
        sel.click("link=Create a new one")
        for i in range(60):
            try:
                if sel.is_element_present("addContributionFocusField"): break
            except: pass
        ...
        sel.type("addContributionFocusField", "bgrt")
        sel.click("//input[@value='Add New']")
        sel.select("//div[3]/div/div/table/tbody/tr/td[2]/div/select", "label=Mr.")
        sel.type("_GID29", "presenter")
        sel.type("//div[3]/div/div/table/tbody/tr[3]/td[2]/div/input", "presenter")
        sel.type("_GID30", "presenter@dummz.org")
        sel.click("css=a > input[type=button]")
        sel.click("css=li.tabUnselected")
        sel.click("//table[2]/tbody/tr/td[2]/div/div/div[2]/input")
        sel.click("css=div.popupButtonBar > div > input[type=button]")
        sel.click("//table[2]/tbody/tr/td[2]/div/div/div[2]/input[2]")
        sel.select("//div[3]/div/div/table/tbody/tr/td[2]/div/select", "label=Mr.")
        sel.type("_GID33", "author")
        sel.type("//div[3]/div/div/table/tbody/tr[3]/td[2]/div/input", "author")
        sel.type("_GID34", "author@dummz.org")
        sel.click("css=a > input[type=button]")
        sel.click("//table[2]/tbody/tr/td[2]/div/div/div[2]/input")
        sel.type("userSearchFocusField", "")
        sel.type("userSearchFocusField", "d")
