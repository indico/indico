...
class ProgrammeTest(LoggedInSeleniumTestCase):
    def setUp(self):
        LoggedInSeleniumTestCase.setUp(self)

    def test_general_settings_test(self):
        sel = self._selenium
        sel.open("/confModifProgram.py?confId=62")
        sel.click("css=input.btn")
        sel.wait_for_page_to_load("30000")
        sel.type("description", "gfedger")
        sel.click("Save")
        sel.wait_for_page_to_load("30000")
        sel.click("//input[@value='add track']")
        sel.wait_for_page_to_load("30000")
        sel.type("title", "track 1")
        sel.click("css=input.btn")
        sel.wait_for_page_to_load("30000")
        sel.click("selTracks")
