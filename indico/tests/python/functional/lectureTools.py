...
class LectureToolsTest(LoggedInSeleniumTestCase):
    def setUp(self):
        LoggedInSeleniumTestCase.setUp(self)

    def test_general_settings_test(self):
        sel = self._selenium
        sel.open("/confModifTools.py?confId=45")
        sel.click("//ul[@id='tabList']/li[4]/a")
        ...
        self.assertEqual("The specified end date is not valid.", sel.get_alert())
        sel.type("css=#cloneDaysPlace_until > div.dateField > input[type=text]", "12/08/2011")
        sel.click("cloneGivenDays")
        sel.wait_for_page_to_load("30000")
        sel.click("confirm")
        sel.wait_for_page_to_load("30000")
        sel.click("css=span.fakeLink")
        sel.click("css=span.listName > a > span")
        sel.wait_for_page_to_load("30000")
