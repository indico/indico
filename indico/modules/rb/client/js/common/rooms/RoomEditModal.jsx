...
function validate(fields) {
  ...
  if (maxAdvanceDays !== null && maxAdvanceDays < 1) {
    errors.maxAdvanceDays = Translate.string(
      'The max. advance booking time must be at least 1 day or empty.'
    );
  }
  if (bookingLimitDays !== null && bookingLimitDays < 1) {
    errors.bookingLimitDays = Translate.string(
      'The max. booking duration must be at least 1 day or empty.'
    );
  }
...
class RoomEditModal extends React.Component {
  ...
  async saveAvailability(roomId, changedValues, nonbookablePeriods, bookableHours) {
    if (nonbookablePeriods || bookableHours) {
      ...
      params = _.fromPairs(
        Object.entries(params).map(([field, values]) => [field, values.map(v => _.omit(v, 'key'))])
      );
  ...
  renderContent = (content, key) => {
    ...
    switch (content.type) {
      case 'header':
        ...
        return <Header key={key}>{content.label}</Header>;
      case 'input': {
        return (
          <FinalInput
            key={key}
      ...
      case 'formgroup': {
        ...
        return (
          <Wrapper {...props}>
            ...
            <Form.Group widths="equal">{content.content.map(this.renderContent)}</Form.Group>
          </Wrapper>
        );
      }
      case 'checkbox':
      ...
      case 'photo':
        return this.newRoom ? null : <RoomPhoto key={key} roomId={roomId} hasPhoto={hasPhoto} />;
      case 'attributes':
    ...
    return (
      <>
        ...
        <Modal.Content scrolling>
          <Form id="room-form" styleName="room-form" onSubmit={fprops.handleSubmit}>
            <Grid columns={3}>
              ...
              <Grid.Column>
                ...
                <Message
                  ...
                  positive
                  hidden={!afterCreation || wasEverDirty}
                >
