...
class CalendarListView extends React.Component {
    ...
    componentDidMount() {
        const {actions: {fetchActiveBookings}} = this.props;
    ...
    componentWillUnmount() {
        const {actions: {clearActiveBookings}} = this.props;
        clearActiveBookings();
    }
            ...
            <Card styleName="booking-card" key={key} onClick={() => openBookingDetails(reservation.id)}>
                <Card.Content>
                    <Card.Header>
                        ...
                        <TooltipIfTruncated>
                            <div styleName="booking-card-header">
                                {room.fullName}
                            </div>
                        </TooltipIfTruncated>
                    </Card.Header>
                ...
                {(isFetchingActiveBookings || hasData) && (
                    <LazyScroll hasMore={rowsLeft > 0}
                        ...
                        {isFetchingActiveBookings && (
                            <CardPlaceholder.Group count={ACTIVE_BOOKINGS_LIMIT} itemsPerRow={4} withImage={false} />
                        )}
                    </LazyScroll>
                )}
                {!isFetchingActiveBookings && !hasData && (
                    <Message info>
                        <Translate>
                            There are no bookings matching the criteria.
                        </Translate>
                    </Message>
                )}
            </div>
