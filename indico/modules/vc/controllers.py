...
def process_vc_room_association(plugin, event, vc_room, form, event_vc_room=None, allow_same_room=False):
    ...
    if event_vc_room.link_type != VCRoomLinkType.event and existing:
        db.session.rollback()
        flash(_("There is already a meeting attached to '{link_object_title}'.").format(
    ...
    def _process(self):
        ...
        if form.validate_on_submit():
            ...
            try:
                ...
                Logger.get('modules.vc').warning('Meeting %r not found. Setting it as deleted.', self.vc_room)
...
class RHVCManageEventRefresh(RHVCSystemEventBase):
    ...
    def _process(self):
        ...
        Logger.get('modules.vc').info('Refreshing meeting %r from event %r', self.vc_room, self.event)
