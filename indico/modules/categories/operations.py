...
def update_category_privacy(category, data):
    log_fields = {
        ...
        'lock_privacy_data': {'title': 'Lock privacy data', 'type': 'bool'},
    }
...
def get_category_privacy(category):
    """Get the privacy settings applying to a category.

    :param category: the category to get the privacy settings for
    :return: `(privacy settings, inherited from category)` where the inherited from category is
             the one holding the settings, or `None` if no category in the chain
             has any privacy settings
    """
    chain = category.chain_query.all()[::-1]
    stored = category_privacy_settings.get_all_multi(chain, no_defaults=True)
    for cat in chain:
        if settings := stored[cat.id]:
            return dict(category_privacy_settings.defaults) | settings, cat
    return dict(category_privacy_settings.defaults), None
