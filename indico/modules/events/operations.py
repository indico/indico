...
def get_event_privacy(event):
    """Get the privacy settings applying to an event.

    :param event: the event to get the privacy settings for
    :return: `(privacy settings, source category)` where the source category is
             the category the settings are inherited from, or `None` if the event
             has its own settings or nothing is set anywhere
    """
    if settings := event_privacy_settings.get_all(event, no_defaults=True):
        return dict(event_privacy_settings.defaults) | settings, None
    if event.category:
        settings, source = get_category_privacy(event.category)
        if source:
            return settings, source
    return dict(event_privacy_settings.defaults), None
