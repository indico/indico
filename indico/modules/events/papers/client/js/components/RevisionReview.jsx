...
export default function RevisionReview({review}) {
  ...
  const additionalProps = !review.comment ? {'data-no-comment': true} : {};
  const wrapper = (
      ...
      <div className="flexrow i-timeline-item-content">
        <div className="i-timeline-item-metadata">
          <div className="f-self-stretch">
            ...
            <time dateTime={serializeDate(review.createdDt, moment.HTML5_FMT.DATETIME_LOCAL)}>
              {serializeDate(review.createdDt, 'D MMM YYYY')}
            </time>
            {review.modifiedDt && (
              <span
                className="review-comment-edited"
                title={serializeDate(review.modifiedDt, 'D MMM YYYY')}
              >
                {' '}
        ...
        >
              ...
              {review.ratings.length > 0 &&
                (review.score !== null && (
                  <>
                    ...
                    {'('}
                    <a className="js-ratings-toggle toggle-link">
                      <span className="js-show-ratings">
                        <Translate>show ratings</Translate>
                      </span>
                      <span className="weak-hidden js-hide-ratings">
          ...
          <div className="i-box-content js-form-container">
            {review.comment && <div className="markdown-text">{review.comment}</div>}
          </div>
        </div>
      </div>
    </div>
