...
export const entriesSlice = createSlice({
  ...
  reducers: {
    ...
    scheduleContribs: (
      state,
      action: PayloadAction<{contribs: UnscheduledContrib[]; gap: number}>
    ) => {
      state = {
        ...state,
        ...operations.scheduleContribs(state, action.payload.contribs, action.payload.gap),
        draggedIds: new Set(),
      };
      // TODO test this
    },
    changeColor: (state, action: PayloadAction<{sessionId?: number; color: string}>) => {
      ...
      state = {...state, ...operations.changeBreakColor(state, action.payload.color)};
      // TODO test this
    },
    undoChange: state => {
