...
class RHSessionsList(RHManageSessionsBase):
    ...
    def _process(self):
        ...
        return WPManageSessions.render_template(
            'management/session_list.html',
            self.event,
...
class RHCreateSession(RHManageSessionsBase):
    ...
    def _get_response(self, new_session):
        ...
        return jsonify_data(sessions=sessions, new_session_id=new_session.id, html=_render_session_list(self.event))
    ...
    def _process(self):
        ...
        form = SessionForm(
            obj=FormDefaults(
                colors=get_random_color(self.event),
...
class RHAPISession(RHManageSessionBase):
    """API for a single session."""

    def _process_GET(self):
        return SessionSchema().jsonify(self.session)

    @use_args(SessionSchema, partial=True)
...
class RHAPISessionRandomColor(RHManageSessionsBase):
    ...
    def _process(self):
        return SessionColorSchema().jsonify(get_random_color(self.event))


class RHAPISessionColors(RHManageSessionsBase):

    def _process(self):
        return SessionColorSchema(many=True).jsonify(get_colors())


class RHAPISessionsInheritedLocation(RHManageEventBase):
    """API for the location parent of the event."""

    def _process_GET(self):
        if Session.allow_location_inheritance:
            return LocationParentSchema().jsonify(self.event)
        return jsonify(None)


class RHAPISessionBlock(RHManageSessionBase):
    """API for a single session block."""

    normalize_url_spec = {'locators': {lambda self: self.session_block}}
...
class RHAPISessionBlocksInheritedLocation(RHManageSessionBase):
    """API for the location parent of the event."""

    def _process_GET(self):
        if SessionBlock.allow_location_inheritance:
            return LocationParentSchema().jsonify(self.session)
        return jsonify(None)
...
class RHSessionProtection(RHManageSessionBase):
    ...
    def _process(self):
        form = SessionProtectionForm(
            obj=FormDefaults(**self._get_defaults()), session=self.session, prefix='session-protection-'
        )
        if form.validate_on_submit():
    ...
    def _get_defaults(self):
        permissions = [
            [serialize_principal(p.principal), list(get_principal_permissions(p, Session))]
...
class RHSessionACLMessage(RHManageSessionBase):
    ...
    def _process(self):
        ...
        return jsonify_template(
            'forms/protection_field_acl_message.html', object=self.session, mode=mode, endpoint='sessions.acl'
        )
...
class RHManageSessionBlock(RHManageSessionBase):
    ...
    def _process(self):
        form = MeetingSessionBlockForm(
            obj=FormDefaults(**self._get_form_defaults()), event=self.event, session_block=self.session_block
        )
        if form.validate_on_submit():
...
class RHManageSessionTypes(RHManageSessionsBase):
    ...
    def _process(self):
        return jsonify_template(
            'events/sessions/management/types_dialog.html', event=self.event, types=self.event.session_types
        )


class RHAPISessionTypes(RHManageSessionsBase):
    """API to list session types of an event."""

    def _process(self):
        return SessionTypeSchema(many=True).jsonify(self.event.session_types)


class RHManageSessionTypeBase(RHManageSessionsBase):
    """Manage a session type of an event."""

    normalize_url_spec = {'locators': {lambda self: self.session_type}}
...
class RHEditSessionType(RHManageSessionTypeBase):
    ...
    def _process(self):
        ...
        if form.validate_on_submit():
            ...
            db.session.flush()
            self.event.log(
                EventLogRealm.management, LogKind.change, 'Sessions', f'Updated type: {old_name}', session.user
            )
...
class RHCreateSessionType(RHManageSessionsBase):
    ...
    def _process(self):
        ...
        if form.validate_on_submit():
            ...
            db.session.flush()
            self.event.log(
                EventLogRealm.management, LogKind.positive, 'Sessions', f'Added type: {session_type.name}', session.user
            )
            types = [{'id': t.id, 'title': t.name} for t in self.event.session_types]
            return jsonify_data(
                types=types, new_type_id=session_type.id, html_row=render_session_type_row(session_type)
            )
        return jsonify_form(form)
...
class RHDeleteSessionType(RHManageSessionTypeBase):
    ...
    def _process(self):
        ...
        self.event.log(
            ...
            LogKind.negative,
            'Sessions',
            f'Deleted type: {self.session_type.name}',
            session.user,
        )
        return jsonify_data(flash=False)
