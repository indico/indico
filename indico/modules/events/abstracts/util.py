...
def build_default_email_template(event: Event, tpl_type: str) -> AbstractEmailTemplate:
    ...
    """
    tpl_path = f'events/abstracts/emails/default_{tpl_type}_notification.txt'
    email = get_template_module(tpl_path)
...
def generate_spreadsheet_from_abstracts(abstracts, static_item_ids, dynamic_items):
    ...
    static_item_mapping = {
        ...
        'authors': ('Authors', lambda x: [a.full_name for a in x.primary_authors]),
        'authors_affiliation': ('Authors (affiliation)',
                                lambda x: [a.full_name_affiliation for a in x.primary_authors]),
        'authors_email': ('Authors (email)', lambda x: _names_with_emails(x.primary_authors)),
