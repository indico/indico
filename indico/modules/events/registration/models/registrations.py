...
class Registration(db.Model):
    ...
    def has_checked_in(self):
        return (RegistrationCheck.query
                .with_parent(self)
                .filter(~RegistrationCheck.is_check_out,
                        RegistrationCheck.check_type_id == self.event.default_check_type_id)
                .has_rows())

    @has_checked_in.expression
    def has_checked_in(cls):
        return (RegistrationCheck.query
                .filter(RegistrationCheck.registration_id == cls.id,
    ...
    def can_perform_check(self, *, check_type=None, is_check_out=False):
        ...
        if is_check_out and not check_type.check_out_allowed:
            ...
            can_check = False
            reason = _('This check type has already been performed once today.')
        return {'can_check': can_check, 'reason': reason}
