...
class GoogleWalletManager:
    ...
    def create_class_template(self, issuer_id: str, class_suffix: str) -> dict:
        ...
        logo_url = f'{config.BASE_URL}{config.IMAGES_BASE_URL}/logo_indico.png'
        if config.LOGO_URL:
            logo_url = config.LOGO_URL if 'http' in config.LOGO_URL else f'{config.BASE_URL}{config.LOGO_URL}'
        dict_template = {
