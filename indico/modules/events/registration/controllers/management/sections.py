...
class RHRegistrationFormModifySection(RHManageRegFormSectionBase):
    ...
    def _process_PATCH(self):
        ...
        if changes.get('is_manager_only') == (False, True):
            ...
            if any(field.show_if_id for field in self.section.children if not field.is_deleted):
                raise ValidationError('Sections with fields used as conditional cannot be made manager-only')
