...
class FieldStats(object):
    ...
    def _build_data(self):
        ...
        for k, regitems in groupby((regitem for regitem in self._regitems if regitem.price), self._build_key):
            choices['billed'][k] = self._build_regitems_data(k, list(regitems))
        for k, regitems in groupby((regitem for regitem in self._regitems if not regitem.price), self._build_key):
