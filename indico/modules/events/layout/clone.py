...
class LayoutCloner(EventCloner):
    ...
    def _copy_menu_entry(self, menu_entry, event):
        base_columns = {column.key for column in inspect(MenuEntry).column_attrs} - {'id', 'event_id', 'parent_id',
                                                                                     'page_id'}
        new_menu_entry = MenuEntry(event_id=event.id, **{col: getattr(menu_entry, col) for col in base_columns})
        if menu_entry.is_page:
            new_page = MenuPage(html=menu_entry.page.html)
            db.session.add(new_page)
            db.session.flush()
            new_menu_entry.page = new_page
