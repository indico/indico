...
class ListGeneratorBase(object):
    ...
    def _get_config(self):
        """Load the lists's configuration from the DB and return it."""
        session_key = self._get_config_session_key()
        if self.static_link_used:
            uuid = request.args['config']
            configuration = ReportLink.load(self.list_event, self.list_link_type, uuid)
    ...
    def generate_static_url(self):
        ...
        if configuration['data']:
            link = ReportLink.create(self.list_event, self.list_link_type, configuration)
