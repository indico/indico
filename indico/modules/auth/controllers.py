...
from indico.core.auth import (login_rate_limiter, login_rate_limiter_user, multipass, signup_rate_limiter,
...
class RHLogin(RH):
    ...
    def _process(self, force):
        ...
        if request.method == 'POST':
            ...
            limiter = self._get_exceeded_rate_limiter()
            if not limiter and form.validate_on_submit():
                limiter = self._get_exceeded_rate_limiter(form.identifier.data)
                if not limiter:
    ...
    def _get_exceeded_rate_limiter(self, identifier=None):
        ...
        if login_rate_limiter.test():
            return None
        elif not login_rate_limiter_user.limits:
            return login_rate_limiter
        elif identifier and not login_rate_limiter_user.test(identifier):
...
class RHRegister(RH):
    ...
    def _get_exceeded_rate_limiter(self, email=None):
        """Return the rate limiter that has been exceeded."""
        if signup_rate_limiter.test():
            return None
        elif not signup_rate_limiter_email.limits:
            return signup_rate_limiter
        elif email and not signup_rate_limiter_email.test(email):
