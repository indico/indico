...
def upgrade():
    op.create_table('logs',
        ...
        sa.ForeignKeyConstraint(['target_user_id'], ['users.users.id'], name=op.f('fk_logs_target_user_id_users')),
        sa.ForeignKeyConstraint(['user_id'], ['users.users.id'], name=op.f('fk_logs_user_id_users')),
        sa.PrimaryKeyConstraint('id', name=op.f('pk_logs')),
        schema='users'
    )
    op.create_index(op.f('ix_logs_meta'), 'logs', ['meta'], unique=False, schema='users', postgresql_using='gin')
