...
from MaKaC.plugins.Collaboration.Vidyo.common import (VidyoException, VidyoError, FakeAvatarOwner, VidyoTools,
                                                      getVidyoOptionValue)
from MaKaC.user import AvatarHolder, Avatar
from MaKaC.plugins.Collaboration.Vidyo.api.operations import VidyoOperations
from MaKaC.plugins.Collaboration.Vidyo.fossils import (ICSBookingConfModifFossil, ICSBookingIndexingFossil)
...
class CSBooking(CSBookingBase):
    ...
    def isLinkedToEquippedRoom(self):
        return self._created and (not VidyoTools.getLinkRoomAttribute(self.getLinkObject(), attName='h323-ip') or
                                  not VidyoTools.getLinkRoomAttribute(self.getLinkObject(), attName='vidyopanorama-id'))
    ...
    def setOwner(self, owner):
        """ Sets the owner of this public room
            :param owner: The owner of this public room. Can be a fossilized
            avatar, an Avatar or a FakeAvatarOwner object
    ...
    def getBookingInformation(self):
        """ For retrieving the ServiceInformation sections dict built for the
    ...
    def _checkBookingParams(self):
        if len(self._bookingParams['roomName'].strip()) == 0:
            raise VidyoException("roomName parameter (" + str(self._bookingParams['roomName']) +
                                 " ) is empty for Vidyo booking with id: " + str(self._id))
        ...
        if len(self._bookingParams['roomDescription'].strip()) == 0:
            raise VidyoException("roomDescription parameter (" + str(self._bookingParams['roomDescription']) +
                                 " ) is empty for Vidyo booking with id: " + str(self._id))

        if self._pin and not self._pin.isdigit():
            raise VidyoException("pin parameter ({0}) is not an integer for Vidyo booking with id: {1}"
                                 .format(self._pin, self._id))

        if self._moderatorPin and not self._moderatorPin.isdigit():
            raise VidyoException("moderator pin parameter ({0}) is not an integer for Vidyo booking with id: {1}"
    ...
    def _generateLinkVideoText(self):
        ...
        if self.hasSessionOrContributionLink():
            ...
            if linkObject is None:
                return _(u"Removed {0}").format(_(u"contribution") if self.isLinkedToContribution() else _(u"session"))
            if self.isLinkedToContribution() and isinstance(linkObject, Contribution):
                ...
                title = linkObject.getSession().getTitle() + (' - ' +
                                                              linkObject.getTitle() if linkObject.getTitle() else '')
                linkVideoText = title + ' (' + self._linkVideoType + ')'
            else:
                linkVideoText = _(u"Link removed")
        else:
            linkVideoText = _(u"Whole event")
    ...
    def _modify(self, oldBookingParams):
        ...
        """
        result = ExternalOperationsManager.execute(self, 'modifyRoom', VidyoOperations.modifyRoom, self,
                                                   oldBookingParams)
        if isinstance(result, VidyoError):
            if result.getErrorType() == 'unknownRoom':
                self.setBookingNotPresent()
    ...
    def _connect(self, force=False):
        ...
        if not confRoomIp and not confRoomPanoramaUser:
        ...
        if connectionStatus.get('isConnected'):
            if connectionStatus.get('roomName') == self.getBookingParamByName('roomName'):
                return VidyoError(
                    'alreadyConnected',
                    'connect',
                    _(u"It seems that the room has been already connected to the room, please refresh the page.")
                )
            if not force:
                # if connect is not forced, give up
                return VidyoError('alreadyConnected', 'connect',
                                  _(u"The room is already connected to some other endpoint. Please refresh the page."))
            else:
                ...
                if connected:
                    return VidyoError('couldntStop', 'connect',
                                      _(u"It seems like we haven't managed to stop the current call. "
                                        u"Please refresh the page and try again."))
                else:
    ...
    def _disconnect(self):
        ...
        if not connectionStatus.get('isConnected'):
            return VidyoError('alreadyDisconnected', 'disconnect',
                              _(u"It seems that the room has been already disconnected, please refresh the page"))
        ...
        while ask_for_more:
            ...
            for room in result:
                ...
                for booking in VidyoTools.getIndexByVidyoRoom().getBookingList(room.roomID):
                    if (av == user or booking.getConference() in user.getLinkTo('conference', 'manager') or
                            user == booking.getConference().getCreator() or
                            RCVideoServicesManager.hasRights(user, booking.getConference(), ['Vidyo']) or
                            user.isAdmin()):
    ...
    def hasToBeDeleted(self, fromDeleteOld, maxDate):
        return (len(VidyoTools.getIndexByVidyoRoom().getBookingList(self._roomId)) == 1 or
                (fromDeleteOld and not self._hasRecentClones(maxDate)))
    ...
    def _delete(self, fromDeleteOld=False, maxDate=None):
        ...
        if self.isCreated():
            ...
            if deleteRemote:
                result = ExternalOperationsManager.execute(self, 'deleteRoom', VidyoOperations.deleteRoom, self,
                                                           self._roomId)
    ...
    def _getLaunchDisplayInfo(self):
        return {'launchText': _(u"Join Now!"),
                'launchLink': str(self.getURL()),
                'launchTooltip': _(u"Click here to join the Vidyo room!")}
    ...
    def getStatusMessage(self):
        ...
        if not self._created:
            return _(u"Booking no longer exists")
        elif status is None:
            return _(u"Public room created")
        elif status:
            return _(u"Conference started")
        elif not status:
            return _(u"Conference stopped")
    ...
    def _sendNotificationToOldNewOwner(self, oldOwner):
        ...
        if isinstance(self.getOwnerObject(), Avatar):
            try:
                ...
                Logger.get('Vidyo').error(("Could not send VidyoOwnerChosenNotification for booking "
                                           "with id {} of event with id {}, exception: {}")
                                          .format(self.getId(), self.getConference().getId(), str(e)))
        ...
        if oldOwner['_type'] == 'Avatar':
            try:
                ...
                Logger.get('Vidyo').error(("Could not send VidyoOwnerRemovedNotification for booking "
                                           "with id {} of event with id {}, exception: {}")
                                          .format(self.getId(), self.getConference().getId(), str(e)))
    ...
    def _sendMail(self, operation):
        ...
        if operation == 'new':
            ...
            try:
                ...
                Logger.get('Vidyo').error(("Could not send NewVidyoPublicRoomNotificationAdmin for booking "
                                           "with id {} of event with id {}, exception: {}")
                                          .format(self.getId(), self.getConference().getId(), str(e)))
            ...
            if VidyoTools.needToSendCleaningReminder():
                try:
                    ...
                    Logger.get('Vidyo').error(("Could not send VidyoCleaningNotification for booking "
                                               "with id {} of event with id {}, exception: {}")
                                              .format(self.getId(), self.getConference().getId(), str(e)))
            ...
            try:
                ...
                Logger.get('Vidyo').error(("Could not send VidyoPublicRoomModifiedNotificationAdmin for booking "
                                           "with id {} of event with id {}, exception: {}")
                                          .format(self.getId(), self.getConference().getId(), str(e)))
        ...
        # this means that the vidyo room has been detached from all events in Indico
            ...
            try:
                ...
                Logger.get('Vidyo').error(("Could not send VidyoPublicRoomRemovalNotificationAdmin for booking "
                                           "with id {} of event with id {}, exception: {}")
                                          .format(self.getId(), self.getConference().getId(), str(e)))
            ...
            if isinstance(self.getOwnerObject(), Avatar):
                try:
                    ...
                    Logger.get('Vidyo').error(("Could not send VidyoRoomDeletedOwnerNotification for booking "
                                               "with id {} of event with id {}, exception: {}")
                                              .format(self.getId(), self.getConference().getId(), str(e)))
