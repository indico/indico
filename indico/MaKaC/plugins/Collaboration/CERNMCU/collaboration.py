...
class CSBooking(CSBookingBase):
    ...
    def getMCUStartTime(self):
        return MCUTime(self.getAdjustedStartDate('UTC') + timedelta(hours = getCERNMCUOptionValueByName("MCU_UTC_offset")))
    ...
    def _checkBookingParams(self):
        ...
        if len(self._bookingParams["name"]) >= 32:
            ...
            while keepAsking and not found:
                ...
                for conference in answer.get("conferences", []):
                    if conference.get("conferenceName", None) == self._bookingParams["name"]:
                        ...
                        if len(remoteDescription) < 31 or not self._bookingParams["description"].startswith(remoteDescription):
                        ...
                        if startTime:
                            adjustedDate = setAdjustedDate(datetimeFromMCUTime(startTime), tz = 'UTC')
                            delta = timedelta(hours = getCERNMCUOptionValueByName("MCU_UTC_offset"))
                            self.setStartDate(adjustedDate - delta)

                        durationSeconds = conference.get("durationSeconds", None)
                        if durationSeconds:
                            self.setEndDate(self.getStartDate() + timedelta(seconds = durationSeconds))
                        else:
