                    ...
                    <td>
                        <table cellpacing="0" cellpadding="2">
                            ...
                            <tr>
                                ...
                                <td bgcolor="white" nowrap id="tdPrimaryPresenter<%= auth_id %>" class="<% if auth_speaker == 'checked': %>tdSelected<%end%><%else:%>tdNotSelected<%end%>" style="-moz-border-radius: 5px 5px 5px 5px; padding-right:6px; width:100%%;">
                                    <input type="hidden" name="auth_primary" value="%(auth_id)s">
                                    <input type="checkbox" id="primary_auth_speaker<%= auth_id %>" name="auth_speaker" value="%(auth_id)s" %(auth_speaker)s onClick="switchPrimaryAuthorBackground('<%= auth_id %>')"> <%= _("This author will be also a <b>presenter</b>")%>
                               </td>
                            </tr>
                        </table>
                    </td>
