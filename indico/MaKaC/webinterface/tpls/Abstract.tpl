...
<tr id="abstracts${abstract.getId()}" style="background-color: transparent;">
    <td valign="top" align="right" width="3%%"><input type="checkbox" name="abstracts" value="${abstract.getId()}"></td>
    % if "ID" in display:
        <td class="CRLabstractLeftDataCell" nowrap>${abstract.getId()} ${comments}</td>
    % endif
    ...
    % if "PrimaryAuthor" in display:
        <% authList = "<br>".join([ auth.getFullName() for auth in abstract.getPrimaryAuthorList() if auth.getFullName()]) %>
        <td class="CRLabstractDataCell">${authList}</td>
    % endif
