...
id = "barDiv_" + str( dayDT ) + "_" + str( bar.startDT.time() )

resvInfo = "%s  -  %s<br />%s<br />%s" % (bar.startDT.time(), bar.endDT.time(), escapeAttrVal( r.bookedForName ), escapeAttrVal( r.reason ) )
...
<div id="<%= id %>" class="<%= barClass %>" style="cursor: pointer; width: <%= width %>px; left: <%= left %>;" onmouseover="domTT_activate(this, event, 'content', '<%= resvInfo %>', 'delay', 100, 'maxWidth', 320, 'styleClass', 'tip' );" onclick="window.location = '<%=resvUrl%>';"></div>
