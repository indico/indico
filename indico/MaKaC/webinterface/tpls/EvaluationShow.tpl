        ...
        <form name="EvaluationShow" action="<%=actionUrl%>" method="POST" onsubmit="return check()">
          ...
          <table class="evalEditPreview">
            ...
            <% for q in questions: %><% i+=1 %>
              <!--question start-->
              <tr <% if i%2==1: %>class="evalEditPreviewTrGrey"<% end %>>
                <td class="displayField">
                  <% if q.isRequired(): %><span style="color: red">*</span><% end %>
                  <%= "%s. %s"%(q.getPosition(), q.getQuestionValue()) %>
                  <% if len(q.getHelp())>0: inlineContextHelp(q.getHelp()) %><% end %>
                </td>
        ...
        <script type="text/javascript">
          ...
          function check(){
            if (isSubmitClicked){
              var success = true;
              <% for q in questions: %><% if q.isRequired(): %>
