<?xml version='1.0'?>
<!-- $Id: sa.xsl,v 1.4 2009/03/13 12:48:28 jose Exp $
...
<xsl:include href="include/datefr.xsl"/>
...
<tr>
	...
	<xsl:if test="count(child::material) != 0">
		<font face="Times" size="+1"><xsl:for-each select="material">
			<xsl:if test="./title!='document' or count(../repno) = 0">
                            <xsl:if test="./title!='Minutes'">
				<a href="{./displayURL}"><xsl:value-of select="./title" disable-output-escaping="yes"/></a>
				<xsl:if test="count(following-sibling::material) != 0"><br/></xsl:if>
                            </xsl:if>
			</xsl:if>
		</xsl:for-each></font>
	</xsl:if>
...
<tr>
	...
	<td valign="top" width="10pt">
		<font face="times" size="+0">
                  <!--<xsl:number from="contribution" level="any" format="a. "/>-->
                  &#x2014;
                </font>
	</td>
	<td valign="top">
	...
	<font face="times" size="+0"><xsl:value-of select="./title" disable-output-escaping="yes"/></font>
        ...
        <xsl:for-each select="material">
        <xsl:if test="./minutesText != ''">
              <br/><xsl:apply-templates select="./minutesText"/>
        </xsl:if>
        </xsl:for-each>
