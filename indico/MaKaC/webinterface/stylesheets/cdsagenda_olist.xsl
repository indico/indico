<?xml version='1.0'?>
<!-- $Id: cdsagenda_olist.xsl,v 1.14 2009/06/19 12:44:11 jose Exp $
...
<tr class="headerselected" bgcolor="#000060">
  ...
  <td valign="top" align="right">
    <xsl:choose>
    <xsl:when test="./description != '' or count(child::chair) != 0 or count(child::material) != 0 or count(child::location) != 0">
...
<tr>
  <td align="center" valign="top" width="1%">
    ...
    <xsl:number from="agenda" level="any" format="1. "/>
    </b>
    </font>
    <xsl:if test="@broadcasturl != ''">
  ...
  <xsl:choose>
  <xsl:when test="count(./location) != 0 and ./location/room!=../location/room">
...
<xsl:template match="convener|speakers">
	...
	<xsl:if test="count(following-sibling) != 0">,<xsl:text disable-output-escaping="yes"> </xsl:text></xsl:if>
	</xsl:for-each>
</xsl:template>
</xsl:stylesheet>
