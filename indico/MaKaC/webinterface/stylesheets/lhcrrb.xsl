<?xml version='1.0'?>
<!-- $Id: lhcrrb.xsl,v 1.13 2009/03/13 12:48:28 jose Exp $
...
<table width="100%" border="0"><tr><td><img src="images/smallcern.png"/></td><td><center><font size="+2" face="Verdana">LHC Resources Review Boards</font></center></td></tr></table>
<br/><br/><br/>
...
<xsl:template match="session">
  ...
  <xsl:if test="substring(./startDate,12,5) != '00:00'">
     - <u><xsl:value-of select="substring(./startDate,12,5)"/></u>
		...
		<font face="verdana" size="+0"><xsl:for-each select="material">
			<xsl:if test="./title!='document' or ../repno = ''">
				<a href="{./displayURL}"><xsl:value-of select="./title" disable-output-escaping="yes"/></a>
				<xsl:if test="count(following-sibling::material[./title != 'document']) != 0"><br/></xsl:if>
			</xsl:if>
		</xsl:for-each></font>
  </td>
  </tr>
  ...
  <font face="verdana" size="+1">
  <hr width="100%"/>
  <xsl:if test="./description != '' and ./description!=' ' and ./description!='&#xA0;'">
        <xsl:apply-templates select="./description"/>
	<hr width="100%"/>
  </xsl:if>
  </font>
...
<xsl:variable name="idt" select="./ID"/>
	...
	<tr>
	        <td colspan="2" valign="top"><font face="verdana">
	        <br/><br/>
	        </font></td>
...
<tr>
	...
	<xsl:otherwise>
	        <xsl:if test="substring(./startDate,12,5) != '00:00'">
	                <font face="verdana" size="+0"><xsl:value-of select="substring(./startDate,12,5)"/></font><xsl:text disable-output-escaping="yes">&#38;nbsp;&#38;nbsp;</xsl:text>
	        </xsl:if>
		<font face="verdana" size="+0"><xsl:number level="single" format="1. "/></font>
	</xsl:otherwise>
	</xsl:choose>
        </td>
	...
	<br/><font face="verdana" size="+0"><xsl:apply-templates select="./abstract"/></font><br/>
	</xsl:if>
	</td>
	...
	<xsl:choose>
	<xsl:when test="count(child::material[./title = 'document']) != 0">
		<xsl:for-each select="material">
			<xsl:if test="./title='document' and ../repno != ''">
				<font face="verdana" size="+0"><a href="{./displayURL}"><xsl:apply-templates select="../repno"/></a></font><br/>
			</xsl:if>
		</xsl:for-each>
	</xsl:when>
	<xsl:otherwise>
	...
	<xsl:if test="count(child::material[./title != 'document']) != 0">
		<font face="verdana" size="+0"><xsl:for-each select="material">
			<xsl:if test="./title!='document' or ../repno = ''">
				<a href="{./displayURL}"><xsl:value-of select="./title" disable-output-escaping="yes"/></a>
				<xsl:if test="count(following-sibling::material[./title != 'document']) != 0"><br/></xsl:if>
...
<tr>
	...
	<td valign="top" width="10pt">
		<font face="verdana"><xsl:number from="contribution" level="any" format="a. "/></font>
	</td>
	<td valign="top">
...
<tr>
	...
	<font face="verdana" size="+0">--- <xsl:value-of select="./name" disable-output-escaping="yes"/> ---</font>
	</td>
</tr>
</xsl:template>
