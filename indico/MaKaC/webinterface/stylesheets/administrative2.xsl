<?xml version='1.0'?>
<!-- $Id: administrative2.xsl,v 1.10 2009/03/13 12:48:27 jose Exp $
...
<tr>
	...
	<xsl:otherwise>
	        <xsl:if test="substring(./startDate,12,5) != '00:00'">
	                <font face="times" size="+0"><xsl:value-of select="substring(./startDate,12,5)"/></font><xsl:text disable-output-escaping="yes">&#38;nbsp;&#38;nbsp;</xsl:text>
	        </xsl:if>
	...
	<xsl:when test="count(child::material[./title = 'document']) != 0">
		<xsl:for-each select="material">
			<xsl:if test="./title='document' and ../repno != ''">
				<font face="Times" size="+1"><a href="{./displayURL}"><xsl:apply-templates select="../repno"/></a></font><br/>
			</xsl:if>
		</xsl:for-each>
	</xsl:when>
	<xsl:otherwise>
