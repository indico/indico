...
<LECTURE>
  <xsl:attribute name="TITLE"><xsl:value-of select="./session/title" /></xsl:attribute>
  <xsl:choose>
    <xsl:when test="./session/speakers/user != ''">
  ...
  <DURATION><xsl:value-of select="./session/duration"/></DURATION>
  <TIME><xsl:value-of select="substring(./session/startDate, 12, 8)" disable-output-escaping="yes"/></TIME>
  <PAR>
