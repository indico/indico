...
# It would be interesting to use any other lib instead of libxml2,
# cause the latter is quite deprecated
# i.e. http://code.google.com/p/html5lib/ ?
...
class ContextHelp:
    ...
    def __mergeInformation(self, information, snippets, count, xpathCtxt):
        ...
        for elem in xpathRes:               
            ...
            elem.newChild(None,'div',content).newProp('class','ctxtInfo')
    ...
    def __doMerge(self, html, wohl):
        ...
        doc =  libxml2.htmlParseDoc(str(html),"UTF-8")        
        ...
        for name in wohl:
            if wohl[name][0] == "tooltip":
                count = self.__mergeTooltip(wohl[name][1:],snippets,count,xpathCtxt)                   
            elif wohl[name][0] == "information":
                count = self.__mergeInformation(wohl[name][1:],snippets,count,xpathCtxt)                                   
        
        docSer = doc.serialize()
        
        if (len(snippets)!=0):
            # stripWrapper is a dirty dirty hack to bypass the problem
            # that libxml2 outputs HTML documents, and no isolated blocks
