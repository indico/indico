...
def get_authors_from_author_index(event, limit):
    ...
    sorted_authors = sorted(((len(values), values[0]) for author, values in author_index.iteritems()
                            if values[0].getEmail() not in fav_users_emails), reverse=True)
    authors = []
    # Check if there is a corresponding Avatar for the specific user and use that.
    for __, author in sorted_authors[:limit]:
