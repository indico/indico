...
class StrToFakeFileHandle:
    '''We need this fake file handle if we encounter python code inside tpl files in wich case we need to provide a file handle to the python convert function. Note that the file works in FIFO mode and that is not possible to seek it.
    '''
...
class SinkFile:
    '''We need this if we don't want to produce any output. This is more or less the same as writting to /dev/null in GNU/Linux'''
    def __init__(self):
        pass
...
class i18nUtil:
    def __init__(self,chunksize=32):
        ...
        # Strips away whitespaces that come before and after. Note that %(foo)s is also
        # considered as beeing a whitespace here
        self._re_whitespace_splitter = re.compile(r'\A(?:[\s]|%\([^\)]*\)s|&nbsp;)*([\s\S]*?)(?:[\s]|%\([^\)]*\)s|&nbsp;)*\Z')
        ...
        self._nonI18nAttributes = ['accept-charset', 'accept', 'action', 'align', 'alink', 'archive', 'background', 'bgcolor', 'border', 'cellpadding', 'cellspacing', 'checked', 'class', 'classid', 'clear', 'code', 'codebase', 'codetype', 'color', 'cols', 'colspan', 'compact', 'coord', 'enctype', 'disabled', 'face', 'for', 'frame', 'frameborders', 'headers', 'height', 'href', 'hspace', 'http-equiv', 'id', 'ismap', 'language', 'link', 'marginheight', 'marginwidth', 'maxlength', 'media', 'method', 'multiple', 'name', 'nohref', 'noresize', 'noshade', 'nowrap', 'object', 'rel', 'rev', 'rows', 'rowspan', 'rules', 'scrolling', 'selected', 'shape', 'size', 'span', 'src', 'start', 'style', 'tabindex', 'target', 'type', 'usemap', 'valign', 'valuetype', 'version', 'vlink', 'vspace', 'width']
        try:
    ...
    def _saveLearningDict(self):
        '''Serialize the dictionary containing answers (y/n) for every string. This allows us to propose default values when the same string is encountered elsewhere'''
    ...
    def _saveNoDict(self):
        '''Serialize the dictionary containing (string, file, linenumber) for which no has been answered. This allows us to directly say no next time we see the same triplet'''
    ...
    def _isNonI18nAttribute(self, attributeName):
        '''Tells us by looking at the attribute name if this will not be a text that will be displayed, like for example class, style, or align. Answers False by default if the tag is unknown
        @param attributeName: The name of the attribute we want to examine
        @return: True if not subjet to i18n by definition, False otherwise
        '''
        return (attributeName in self._nonI18nAttributes)
    ...
    def _getInnerI18nFromStr(self,strAndLineNb):
        ...
        '''Searches for _() inside a string, and return a list of inner strings, or return a list with the main string if no inner strings where found'''
    ...
    def _replaceDecide(self, toreplace, interactive, filename, linenumber,always_ask=False,dont_use_default=False):
    ...
    def _replaceDecideInteractive(self, toreplace, filename, linenumber,always_ask=False,dont_use_default=False):
        '''Decide wether a given string needs to be replaced based on user input
        @param toreplace: The string to be decided about
        @return: A Boolean
        '''
        default = None
    ...
    def _isSubjectToReplacement(self,toreplace):
        '''Performs a very basic check if this string may have to be repalced. If it is only constitued by spaces, and newlines, forget it
    ...
    def _splitAwayLeadingAndTrailingWhitspaces(self, string):
        '''Splits a given string into tree parts: The leading spaces, The text and The trailling spaces
    ...
    def _convertJSString(self,string, filename, linenumber, interactive=True, recordI18nStr=False,always_ask=False,dont_use_default=False):
        '''Finds translatable strings in a javascript string. This is not so much complicated, but we have to deal with <%= ... %> constructs
        ...
        @param interactive: Wether we ask the user if he wants to replace a string or wether to use heuristics
        ...
        while inputBuffer != '':
            ...
            if beg != 0:
                ...
                (left, text, right) = self._splitAwayLeadingAndTrailingWhitspaces(string)
                ...
                opencodetag = inputBuffer[:3]
                pythoncode = inputBuffer[3:end]
                inputBuffer = inputBuffer[end + 2:]
                inputFakeFile = StrToFakeFileHandle(pythoncode,filename, linenumber)
                outpoutFakeFile = StrToFakeFileHandle("")
                i18nstr = False
                self.convertPY(inputFakeFile, outpoutFakeFile, interactive, recordI18nStr)
                outputBuffer += opencodetag
                outputBuffer +=outpoutFakeFile.read()
                outputBuffer += '%>'
        return outputBuffer

    def _convertJS(self,jscode,filename, linenumber, interactive=True,recordI18nStr=False,always_ask=False,dont_use_default=False):
        '''Finds all strings in javascript code embedded in a template file and replaces them using this syntax '<%= _('string')%>' or "<%= _("string")%>"
        @param jscode: The javascript code to transform
        @param interactive: Wether to ask the user if he wants to replace a string or wether to do it based on heuristics
        ...
        while inputBuffer != '':
            if recordString:
                ...
                output = self._convertJSString(string, filename, linenumber, interactive, recordI18nStr, always_ask, dont_use_default)
                outputBuffer += output
    ...
    def convertPY(self, filein, fileout=sys.stdout, interactive=True,recordI18nStr=False,always_ask=False,dont_use_default=False):
        ...
        @param fileout: An open file descriptor where to send the output
        ...
        # Start line number. This is usefull when the convertTPL() function
        ...
        def unwriteUR(writebuffer):
            '''Looks at the two last written chars, and removes them if they are a possible prefix from a string. We need this because it is only at the moment we know that we have a string, that we can know that these letters have a special meaning
        ...
        def checkI18n(prefixnospaces, suffixwithspaces):
            '''Serches for _( at the end of the buffer. We need to have a second buffer with spaces removed, in case there are too much spaces and _( disapears from inside the buffer
        ...
        while read != '':
            ...
            while read != '':
                ...
                if inComment:
                    ...
                    if countDelimiter == 0:
                        # Check wether the '#' is the first interesting symbol,
                        # and wether we did already start a string
                        if (nextComment != -1) and ((nextSingleQuote == -1) or (nextComment < nextSingleQuote)) and ((nextDoubleQuote == -1) or (nextComment < nextDoubleQuote)) and (countDelimiter == 0):
                            ...
                            if nextDoubleQuote == -1:
                                ...
                                if nextSingleQuote < nextDoubleQuote:
                                    ...
                                    towrite += read[:nextDoubleQuote]
                                    read = read[nextDoubleQuote + 1:]
                    elif read[0] == delimiter and countDelimiter <= 6:
                        ...
                        if nextEscapeChar == -1:
                            ...
                            if nextQuote[delimiter] == -1:
                                ...
                                if nextQuote[delimiter] < nextEscapeChar:
                                    ...
                                    string += read[:nextEscapeChar + 2]
                                    read = read[nextEscapeChar + 2:]
                    elif read[0] == delimiter and countDelimiter <= 3:
                        countDelimiter += 1
                        read = read[1:]
                    else:
                        if countDelimiter < 3:
        ...
        # Write out learnin dict
        self._saveLearningDict()
        # Write out no dict
        self._saveNoDict()


    def convertTPL(self, filein, fileout=sys.stdout, interactive=True,recordI18nStr=False,always_ask=False,dont_use_default=False):
        ...
        attributeName = ''
        # Determines wether the last tag examines was an opening script tag
        isScriptTag = False
        ...
        while read != '':
            ...
            while read != '':
                if recordString:
                    ...
                    # this is an opening script tag, since it recieves a
                    # special threatement
                    if len(read) < 8:
                        read += filein.read(self._chunksize)
                    if not readTagName:
                    ...
                    if first == None:
                        ...
                        if self._isNonI18nAttribute(attributeName) and recordI18nStr == False:
                            fileout.write(attr)
                        else:
                            if attributeName[:2].lower() == 'on':
                                # If our attribute starts with on, this means that it is javascript code, in wich case we threat it differently
                                ...
                                outpoutFakeFile = StrToFakeFileHandle('')
                                self.convertTPL(inputFakeFile, outpoutFakeFile, interactive, recordI18nStr, always_ask, dont_use_default)
