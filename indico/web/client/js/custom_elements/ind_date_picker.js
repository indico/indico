  ...
  class extends CustomElementBase {
    ...
    setup() {
      ...
      openCalendarButton.addEventListener('click', () => {
        // Disable so it doesn't re-open the calendar when
        // clicked while calendar is still open.
      ...
      indCalendar.addEventListener('x-select', evt => {
        setNativeInputValue(input, formatDate(this.format, new Date(evt.target.value)));
        input.select();
        input.focus();
...
function openDialog(dialog, input) {
  dialog.toggleAttribute('data-position-check');
  requestAnimationFrame(() => {
    const inputRect = input.getBoundingClientRect();
    const dialogRect = dialog.firstElementChild.getBoundingClientRect();
    const dialogBottom = inputRect.bottom + dialogRect.height;
    const dialogWillFitBelow = dialogBottom < window.innerHeight;
    dialog.style.setProperty('--dialog-bottom', dialogWillFitBelow ? 'auto' : '100%');
    dialog.toggleAttribute('data-position-check');
    dialog.open = true;
  });
}
