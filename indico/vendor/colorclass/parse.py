...
from .codes import BASE_CODES, ANSICodeMapping


CODE_GROUPS = (
    tuple({str(i) for i in BASE_CODES.values() if i and (40 <= i <= 49 or 100 <= i <= 109)}),  # bg colors
    tuple({str(i) for i in BASE_CODES.values() if i and (30 <= i <= 39 or 90 <= i <= 99)}),  # fg colors
...
def prune_overridden(ansi_string):
    ...
    :rtype: str
    """
    multi_seqs = {p for p in RE_ANSI.findall(ansi_string) if ';' in p[1]}  # Sequences with multiple color codes.
...
def parse_input(tagged_string, keep_tags):
