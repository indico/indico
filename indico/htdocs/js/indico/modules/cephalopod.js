...
(function(global) {
    ...
    global.setupCephalopodSettings = function setupCephalopodSettings(cephalopodUrl, enabled) {
        ...
        _checkSynced(cephalopodUrl, {
            enabled: $('#joined').prop('checked'),
            contact: $('#contact_name').val(),
            email: $('#contact_email').val(),
            url: $('#server-url').html(),
            organization: $('#affiliation').html()
        }, true).done(function(synced) {
            if (synced) {
                dfd.resolve({
                    title: $T.gettext('Synchronized'),
                    subtitle: $T.gettext('This server is registered as part of the community and is up to date.'),
                    class: 'highlight',
                    icon: 'icon-checkmark'
                });
            } else {
                ...
                dfd.resolve({
                    ...
                    subtitle: $T.gettext('Your Indico server is out of sync. Synchronize it with the button on the right.'),
                    class: 'warning',
                    icon: 'icon-warning'
                });
            }
    ...
    global.checkCephalopodSynced = function checkCephalopodSynced(cephalopodUrl, checkData, settingsUrl) {
