...
var RegFormFieldsView = Backbone.View.extend({
    ...
    render: function(field, sectionId) {
        ...
        if ($.inArray(field.input, allowedType) == -1) {
        ...
        //         break;
        //     case 'label':
        //         fieldHtml = this._getTplField(field.input, {classes: regForm.classes, field: field});
        //         break;
        //     case 'yes/no':
        //         field.input = 'yesno';
        ...
        //         fieldHtml = this._getWrapper(field, sectionId);
        // }
        var tpl = this._getTplPath(field);
        var dict = {
            attributes: this._getAttributes(field),
    ...
    _getTplPath: function(field) {
        tpl = field.input == "yes/no" ? "yesno" : field.input;
        return this.tplDir + tpl + this.tplExt;
    },

    _getAttributes: function(field) {
        var attributes = "";
    ...
    //     if (field.input == 'date') {
    //         return '_genfield_' + sectionId + '_' + field.id;
    //     } else {
    //         return '*genfield*' + sectionId + '-' + field.id;
    //     }
    // }
});
...
var RegFormDisplayView = Backbone.View.extend({
    ...
    initialize: function(el) {
        ...
        var templates = ['regFormSection', 'regFormFields'];
    ...
    getSection: function(dict) {
        ...
        // var tpl = TemplateManager.getCached('RegistrationForm','regFormSection', false);

        // TODO change id in section model
        if (dict.section.id == "accommodation") {
            tpl += "accommodation";
        } else if (dict.section.id == "furtherInformation") {
            tpl += "furtherinfo";
        } else if (dict.section.id == "reasonParticipation") {
            tpl += "reason";
        } else if (dict.section.id == "sessions") {
            tpl += "sessions";
        } else if (dict.section.id == "socialEvents") {
            tpl += "socialevents";
        } else {
            tpl += "general";
            dict.fields = [];
            _.each(dict.section.items, function(item) {
                dict.fields.push(self.rfFields.render(item, dict.section.id));
            });
            dict.calendarimg = imageSrc("CalendarWidget");
        }
        ...
        _.each(data.miscellaneousGroupList, function(sectionUD) {
            ...
            _.each(sectionUD.responseItems, function(item) {
                ...
                if (fieldType == 'checkbox') {
                    ...
                    var fieldSelected = _.find(field, function(el) {
                        return $(el).attr('value') == id;
                    });
